...
import OperatorStatistics from "@/components/operator-statistics"
...
export default function CustomerStatistics() {
  ...
  const filteredCustomers = Array.from(customerMap.entries())
    .filter(([customer]) => customer.toLowerCase().includes(searchTerm.toLowerCase()))
    .map(([customer, assignments]) => {
      ...
      return [customer, filteredAssignments] as [string, typeof customerAssignments]
    })
    ...
    .sort(([, assignmentsA], [, assignmentsB]) => {
      ...
      const latestA = new Date(Math.max(...assignmentsA.map((a) => new Date(a.assigned_at).getTime())))
      const latestB = new Date(Math.max(...assignmentsB.map((a) => new Date(a.assigned_at).getTime())))
  ...
  const filteredStats = {
    ...
    totalAssignments: filteredCustomers.reduce((sum, [, assignments]) => sum + assignments.length, 0),
    privateAccounts: filteredCustomers.reduce(
      (sum, [, assignments]) => sum + assignments.filter((a) => a.account_type === "private").length,
      0,
    ),
    sharingAccounts: filteredCustomers.reduce(
      (sum, [, assignments]) => sum + assignments.filter((a) => a.account_type === "sharing").length,
      0,
    ),
  }
  ...
  const exportToExcel = () => {
    ...
    // Create CSV content
    const headers = ["Customer", "Email", "Type", "Profile", "Operator", "Date"]
    const csvContent = [
      headers.join(","),
      ...data.map((row) =>
        headers
          .map((header) => {
            ...
            const cell = String(row[header as keyof typeof row]).replace(/"/g, '""')
            return `"${cell}"`
          })
          .join(","),
      ),
  ...
  return (
    <Tabs defaultValue="customers" className="w-full">
      <TabsList className="grid w-full grid-cols-2 mb-4">
        ...
        {isAdmin && <TabsTrigger value="operators">Operator Statistics</TabsTrigger>}
      </TabsList>
      ...
      <TabsContent value="customers">
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="flex items-center">
                  ...
                  <div className="text-2xl font-bold">{filteredStats.totalCustomers}</div>
                  {dateRange && <span className="text-xs text-gray-500 ml-2">(filtered)</span>}
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center">
                  ...
                  <div className="text-2xl font-bold">{filteredStats.totalAssignments}</div>
                  {dateRange && <span className="text-xs text-gray-500 ml-2">(filtered)</span>}
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-gray-500">Private Accounts</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-center">
                  <BarChart className="h-5 w-5 text-gray-500 mr-2" />
                  <div className="text-2xl font-bold">{filteredStats.privateAccounts}</div>
                  {dateRange && <span className="text-xs text-gray-500 ml-2">(filtered)</span>}
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-gray-500">Sharing Accounts</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-center">
                  <PieChart className="h-5 w-5 text-gray-500 mr-2" />
                  <div className="text-2xl font-bold">{filteredStats.sharingAccounts}</div>
                  {dateRange && <span className="text-xs text-gray-500 ml-2">(filtered)</span>}
                </div>
              </CardContent>
            </Card>
                  ...
                  <TableBody>
                    {filteredCustomers.length === 0 ? (
                      ...
                      filteredCustomers.map(([customer, assignments]) => {
                        ...
                        return (
                          <TableRow key={customer} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                              ...
                              <div className="space-y-1">
                                {sortedAssignments.map((assignment, index) => (
                                  <div key={index} className="flex items-center">
                                    ...
                                    <span className="text-sm">{assignment.account_email}</span>
                            ...
                            <TableCell>
                              <Badge variant="outline">{latestAssignment.operator_name || "Unknown"}</Badge>
                            </TableCell>
                            <TableCell>
