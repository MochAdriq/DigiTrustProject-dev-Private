...
export default function AccountList({ type }: AccountListProps) {
  const { getAccountsByType, getRemainingDays, deleteAccount, isLoading } = useAccounts()
  ...
  return (
    <>
        ...
        <CardContent className="pt-6">
          {filteredAccounts.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-500 dark:text-gray-400">No {type} accounts found</p>
            </div>
          ) : (
              ...
              <Table>
                ...
                <TableBody>
                  {filteredAccounts.map((account) => {
                    const availableProfiles = account.profiles.filter((p) => !p.used).length
                    ...
                    return (
                      <TableRow key={account.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                        ...
                        <TableCell>
                          <Badge
                            className={daysLeft <= 3 ? "bg-red-500" : daysLeft <= 7 ? "bg-yellow-500" : "bg-green-500"}
                          >
                            {daysLeft} days
                          </Badge>
                        </TableCell>
      ...
      {editingAccount && (
        <EditAccountDialog account={editingAccount} open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen} />
      )}
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the account and all its data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
